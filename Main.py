...
from config import (API_KEY, OWNER_USERNAME, PROMPT_FIRST_TEMPLATE, PROMPT_SECOND_TEMPLATE,
                    ...
                    NOTIFICATION_CHECK_INTERVAL_MIN, NOTIFICATION_CHECK_INTERVAL_MAX)  # SESSION_ID removed
...
d_device_identifier = "192.168.29.104:39935"
...
def auto_respond_via_ui():
    ...
    while True:
        sleep_after_cycle = True  # Flag to determine if long sleep is needed
        try:
            print(f"\n--- {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} --- UI Bot Cycle ---")

            # 1. Check for notifications first
            found_notification = u2_utils.check_for_instagram_dm_notification(d_device)

            if found_notification:
                print("Instagram DM notification detected, proceeding to check DMs.")
                # Ensure we are in a good state (DM list) before processing.
                # The notification click might land us in a specific chat or elsewhere.
                if not u2_utils.go_to_dm_list(d_device):
                    print("Failed to navigate to DM list after notification. Retrying after short sleep.")
                    time.sleep(30)  # Short sleep before retrying cycle
                    continue  # Restart the loop to check notifications again

                # DM processing logic (similar to before)
                threads_on_screen = u2_utils.get_threads_from_dm_list(d_device, BOT_ACTUAL_USERNAME,
                                                                      THREAD_FETCH_AMOUNT)

                if not threads_on_screen:
                    print(
                        "No threads found on DM screen even after notification. Possibly a misfire or already handled.")
                    # No need for long sleep here, loop will restart and check notifications again quickly.
                    sleep_after_cycle = False  # Don't do the long sleep
                else:
                    active_thread_identifier = None
                    for thread_summary in threads_on_screen:
                        thread_identifier = thread_summary["id"]
                        if thread_identifier.lower() == BOT_ACTUAL_USERNAME.lower(): continue
                        print(
                            f"\nProcessing UI Thread: {thread_identifier} (triggered by notification or general check)")
                        ...
                        for msg_ui in messages_in_thread_ui:
                            ...
                            if msg_ui[
                                "timestamp"] > last_ts_for_thread and ui_msg_tuple_id not in processed_message_ids:
                        ...
                        if not new_ui_messages_to_process:
                            ...
                            active_thread_identifier = None
                            # No u2_utils.go_to_dm_list here, it's done after the loop for threads
                            time.sleep(1)  # Small pause before processing next thread from list
                            # This continue is for the inner loop (messages in thread), not outer (threads_on_screen)
                            # We still need to go back to DM list after this thread is done.
                            if not u2_utils.return_to_dm_list_from_thread(d_device):
                                u2_utils.go_to_dm_list(d_device)
                            continue  # To the next thread_summary
                        ...
                        for message_data in new_ui_messages_to_process:
                            ...
                            if ui_msg_tuple_id_for_processing in processed_message_ids: continue
                            ...
                            print(
                                f"New UI DM in {thread_identifier} from {sender_username_ui}: {message_text_ui[:40]}...")

                            if not auto_responding.get(thread_identifier, True):
                                # ... (auto-response pause check logic remains the same)
                            ...
                            prompt_history = []
                            # ... (history building logic remains the same)
                            ...
                            for hist_msg in sorted_thread_msgs[-10:]:
                                role = "User" if hist_msg[
                                                     "username"].lower() != BOT_ACTUAL_USERNAME.lower() else "Raphael"
                            ...
                            prompt_first = format_message_for_llm(
                                PROMPT_FIRST_TEMPLATE,  # ... (arguments remain the same)
                            ...
                            for part in response_first.parts:
                                if part.function_call:
                                    # ... (function call handling logic remains the same)
                                    ...
                                    if llm_function_name == "notify_owner":
                                        ...
                                        send_message_to_owner_via_ui(llm_args_for_second_prompt.get("message",
                                                                                                    "LLM requested owner notification."),
                                                                     original_ctx)
                                        llm_args_for_second_prompt[
                                            "details_for_user"] = f"I've notified my owner, {OWNER_USERNAME}."
                                    elif llm_function_name == "send_message":
                                        # ... (send_message logic remains the same)
                                        msg_to_send = llm_args_for_second_prompt.get("message")
                                        target_user = llm_args_for_second_prompt.get("target_username")
                                        target_thread_id = llm_args_for_second_prompt.get("thread_id")
                                        actual_target = target_user or target_thread_id
                                        success_send = False
                                    ...
                                    # ... (direct reply logic remains the same)
                                    ...
                                    if active_thread_identifier.lower() == thread_identifier.lower():
                                        ...
                                        if u2_utils.open_thread_by_username(d_device, thread_identifier):
                                            ...
                                            print(
                                                f"ERROR: Could not re-open {thread_identifier} to send LLM direct reply.")

                            if function_triggered_this_message and llm_function_name:
                                # ... (second prompt logic remains the same)
                                ...
                                prompt_second = format_message_for_llm(
                                    PROMPT_SECOND_TEMPLATE,  # ... (arguments remain the same)
                                    bot_display_name=BOT_DISPLAY_NAME, bot_actual_username=BOT_ACTUAL_USERNAME,
                        ...
                        if not u2_utils.return_to_dm_list_from_thread(d_device):
                            u2_utils.go_to_dm_list(d_device)
                        active_thread_identifier = None
                        time.sleep(1)  # Small pause before next thread_summary
                    # End of for thread_summary in threads_on_screen loop
                    sleep_after_cycle = False  # Processed DMs, so quick re-check for notifications

            else:  # No notification found
                print("No new Instagram DM notification detected.")
                # Sleep for the shorter notification check interval
                sleep_duration = random.randint(NOTIFICATION_CHECK_INTERVAL_MIN, NOTIFICATION_CHECK_INTERVAL_MAX)
                print(f"Sleeping for {sleep_duration} seconds (notification check interval).")
            ...
            try:
                ...
                time.sleep(10)
            sleep_after_cycle = True  # Error occurred, do a longer sleep

        if sleep_after_cycle:  # This will only be true if there was an error, or if notification processing was skipped entirely (e.g. failed go_to_dm_list at start)
            sleep_duration = random.randint(MIN_SLEEP_TIME, MAX_SLEEP_TIME)
            print(f"Sleeping for {sleep_duration} seconds (long interval after error or no activity).")
            time.sleep(sleep_duration)
